...
export class Curve extends Component {
    ...
    toTikZ() {
        ...
        return `\\draw[draw=${this.state.strokeColor.tikzColor()}, draw opacity=${this.state.strokeColor.tikzAlpha()}, line width=${this.state.strokeWidth/2}, fill=${this.state.fillColor.tikzColor()}, fill opacity=${this.state.fillColor.tikzAlpha()},${this.state.arrow}] ${pointsFormat} ;\n`
    }
    ...
    render() {
        ...
        return (
            <g ref={el=>this.node=el} id={this.state.id}>
                ...
                <path d={def} style={style} markerEnd={this.state.arrow !== '' ? markerRef: ''} markerStart={this.state.arrow === '<->'? markerRef: ''} strokeDasharray={strokeStyle}/>
                {anchorLines}
                {anchorPoints}
                {controlPoints}
            </g>
